...
[{
    ...
    _trackedUnits = if (GRAD_REPLAY_AI_ONFOOT_TRACKED) then {
        allUnits + switchableUnits
    } else {
        if (GRAD_REPLAY_AI_VEHICLES_TRACKED) then {
            playableUnits + switchableUnits + allDead + (vehicles select {!(_x isKindOf "Thing")})
        } else {
            playableUnits + switchableUnits + allDeadMen
        };
    };
